...
The fact that the similarity is non-uniform in part reflects that these two datasets are simply different; we don't expect all the DIM candidates to reflect safety.
...
== Layer-wise Activation Divergence: ActSVD vs. Difference-in-Means
...
On the other hand, the fact that any one of these methods can be successful without touching the other methods' safety subspaces shows that these safety mechanisms are brittle.
