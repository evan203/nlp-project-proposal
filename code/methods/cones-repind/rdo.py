...
def train_refusal_vector(group_name=None, run_name=None, orthogonal_vectors=[], **kwargs):
    ...
    with wandb.init(project=os.getenv("WANDB_PROJECT"),
        ...
        local_run_dir = os.path.join(SAVE_DIR, "local_vectors", group_name or "rdo", wandb.run.name or wandb.run.id)
        os.makedirs(local_run_dir, exist_ok=True)
        torch.save(results["lowest_loss_vector"].detach().cpu(), os.path.join(local_run_dir, "lowest_loss_vector.pt"))
        torch.save([v.detach().cpu() for v in results["vectors"]], os.path.join(local_run_dir, "vectors.pt"))
        with open(os.path.join(local_run_dir, "metadata.json"), "w") as f:
            json.dump({"group": group_name, "run": wandb.run.name, "wandb_id": wandb.run.id, "config": wandb_config}, f, indent=2, default=str)
...
def train_refusal_cone(group_name, run_name, init_vectors, **kwargs):
    ...
    with wandb.init(project=os.getenv("WANDB_PROJECT"),
        ...
        local_run_dir = os.path.join(SAVE_DIR, "local_vectors", group_name or "rco", wandb.run.name or wandb.run.id)
        os.makedirs(local_run_dir, exist_ok=True)
        torch.save(results["lowest_loss_vector"].detach().cpu(), os.path.join(local_run_dir, "lowest_loss_vector.pt"))
        torch.save([v.detach().cpu() for v in results["vectors"]], os.path.join(local_run_dir, "vectors.pt"))
        with open(os.path.join(local_run_dir, "metadata.json"), "w") as f:
...
def train_independent_vector(group_name=None, run_name=None, independent_vectors=None, **kwargs):
    ...
    with wandb.init(project=os.getenv("WANDB_PROJECT"),
        ...
        local_run_dir = os.path.join(SAVE_DIR, "local_vectors", group_name or "repind", wandb.run.name or wandb.run.id)
        os.makedirs(local_run_dir, exist_ok=True)
        torch.save(results["lowest_loss_vector"].detach().cpu(), os.path.join(local_run_dir, "lowest_loss_vector.pt"))
        torch.save([v.detach().cpu() for v in results["vectors"]], os.path.join(local_run_dir, "vectors.pt"))
        with open(os.path.join(local_run_dir, "metadata.json"), "w") as f:
